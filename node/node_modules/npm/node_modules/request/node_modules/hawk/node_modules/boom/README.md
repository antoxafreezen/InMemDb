...
- HTTP 4xx Errors
  ...
  - 401: [`Boom.unauthorized([message], [database], [attributes])`](#boomunauthorizedmessage-database-attributes)
...
### `Boom.unauthorized([message], [database], [attributes])`

Returns a 401 Unauthorized error where:
- `message` - optional message.
- `database` can be one of the following:
  - an authentication database name
...
If either `database` or `attributes` are set, the resultant `Boom` object will have the 'WWW-Authenticate' header set for the response.
