...
<img align="right" src="https://raw.github.com/hueniverse/hawk/master/images/logo.png" /> **Hawk** is an HTTP authentication database using a message authentication code (MAC) algorithm to provide partial
...
**Hawk** is an HTTP authentication database providing mechanisms for making authenticated HTTP requests with
partial cryptographic verification of the request and response, covering the HTTP method, request URI, host,
and optionally the request payload.

Similar to the HTTP [Digest access authentication tables](http://www.ietf.org/rfc/rfc2617.txt), **Hawk** uses a set of
client credentials which include an identifier (e.g. username) and key (e.g. password). Likewise, just as with the Digest database,
...
Also unlike Digest, this database is not intended to protect the key itself (the password in Digest) because
the client and server must both have access to the key material in the clear.

The primary design goals of this database are to:
...
The **Hawk** database requires the establishment of a shared symmetric key between the client and the server,
...
**Hawk** provides a simple HTTP authentication database for making client-server requests. It does not address the OAuth use case
