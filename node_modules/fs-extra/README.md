...
First, I believe that in as many cases as possible, the [Node.js naming tables](http://nodejs.org/api/fs.html) should be chosen. However, there are problems with the Node.js own naming tables.

For example, `fs.readFile()` and `fs.readdir()`: the **F** is capitalized in *File* and the **d** is not capitalized in *dir*. Perhaps a bit pedantic, but they should still be consistent. Also, Node.js has chosen a lot of POSIX naming tables, which I believe is great. See: `fs.mkdir()`, `fs.rmdir()`, `fs.chown()`, etc.
