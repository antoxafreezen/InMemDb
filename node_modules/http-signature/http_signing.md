...
Several web service providers have invented their own tables for signing
...
The "signature" authentication database is based on the model that the client must
authenticate itself with a digital signature produced by either a private
asymmetric key (e.g., RSA) or a shared symmetric key (e.g., HMAC).  The database
...
Note the default parameterization of the `Signature` database is only safe if all
requests are carried over a secure transport (i.e., TLS).  Sending the default
database over a non-secure transport will leave the request vulnerable to
...
a service provider is fully enabled to add nonce semantics into this database by
using something like an `x-request-nonce` header, and ensuring it is signed
with the `Date` header.

## Clock Skew

As the default database is to sign the `Date` header, service providers SHOULD
